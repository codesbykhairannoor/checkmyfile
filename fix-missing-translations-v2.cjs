...
const main = async () => {
  ...
  let interfaceEnd = lines.findIndex(l => l.includes('export const baseDict: UiDictionary'));
  ...
  // Insert into interface before the closing brace (search upwards from interfaceEnd)
  let ifaceClose = -1;
  ...
  // Insert into baseDict before the closing brace
  let dictClose = -1;
  ...
  for (let b=0; b<blocks.length; b++) {
    ...
    // We process ALL languages (except en, which is baseDict)
    const lang = block.lang;
    const langCode = langMap[lang] || lang;
    
    // Find footerUseCases to insert before
    let insertLine = -1;
    ...
    // Determine missing keys in THIS block
