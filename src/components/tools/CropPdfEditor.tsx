    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ padding: '24px', borderBottom: '1px solid var(--border-color)', background: 'rgba(255,255,255,0.02)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
          ...
          <h2 style={{ fontSize: '1.25rem', fontWeight: 800, margin: 0 }}>{tUi["Crop Margin PDF"] || (tUi["Crop Margin PDF"] || "Crop Margin PDF")}</h2>
        </div>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', margin: 0 }}>{tUi["Potong pinggiran putih pada halaman."] || (tUi["Potong pinggiran putih pada halaman."] || "Potong pinggiran putih pada halaman.")}</p>
      </div>

      <div style={{ padding: '24px', flex: 1, overflowY: 'auto' }}>
        <div className="glass-panel" style={{ padding: '20px' }}>
          <h3 style={{ fontSize: '0.95rem', fontWeight: 700, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
            <Settings size={16} /> {tUi["Pengaturan Margin (%)"] || "Pengaturan Margin (%)"}</h3>

          <div style={{ display: 'grid', gap: 16 }}>
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 600 }}>{tUi["Margin Atas"] || (tUi["Margin Atas"] || "Margin Atas")}</label>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 600 }}>{tUi["Margin Bawah"] || (tUi["Margin Bawah"] || "Margin Bawah")}</label>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 600 }}>{tUi["Margin Kiri"] || (tUi["Margin Kiri"] || "Margin Kiri")}</label>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 600 }}>{tUi["Margin Kanan"] || (tUi["Margin Kanan"] || "Margin Kanan")}</label>
          ...
          <div style={{ marginTop: 24, padding: 12, background: 'rgba(59, 130, 246, 0.1)', borderRadius: 8, fontSize: '0.8rem', color: '#3b82f6', display: 'flex', alignItems: 'flex-start', gap: 8 }}>
            ...
            <p style={{ margin: 0, lineHeight: 1.5 }}>{tUi["Geser nilai di atas untuk menyesuaikan ukuran margin yang akan dipotong. Pratinjau hasil akan ditampilkan setelah Anda menekan tombol di bawah."] || (tUi["Geser nilai di atas untuk menyesuaikan ukuran margin yang akan dipotong. Pratinjau hasil akan ditampilkan setelah Anda menekan tombol di bawah."] || "Geser nilai di atas untuk menyesuaikan ukuran margin yang akan dipotong. Pratinjau hasil akan ditampilkan setelah Anda menekan tombol di bawah.")}</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '24px', borderTop: '1px solid var(--border-color)', background: 'var(--bg-card)' }}>
        <button onClick={onApply} disabled={isProcessing} className="btn-primary" style={{ width: '100%', padding: '14px', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
          {isProcessing ? (
            ...
            <><Crop size={20} />{tUi["Potong PDF Sekarang"] || (tUi["Potong PDF Sekarang"] || "Potong PDF Sekarang")}</>
          )}
        </button>
      </div>
    </div>
