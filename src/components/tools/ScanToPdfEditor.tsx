  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, background: 'var(--bg-card)', padding: 24, borderRadius: 16, border: '1px solid var(--border-color)', boxShadow: '0 4px 12px rgba(0,0,0,0.02)' }}>
        ...
        <div>
          <h3 style={{ margin: 0, marginBottom: 8, fontSize: '1.25rem', fontWeight: 800, color: 'var(--text-main)', fontFamily: 'var(--font-display)' }}>{tUi["Rasterize PDF (Jadikan Permanen)"] || (tUi["Rasterize PDF (Jadikan Permanen)"] || "Rasterize PDF (Jadikan Permanen)")}</h3>
          <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '0.95rem', lineHeight: 1.5 }}>{tUi["Sistem akan menggambar ulang setiap teks dan komponen PDF menjadi gambar statis beresolusi tinggi, membuat dokumen ini 100% aman dari proses pengeditan atau salin-tempel."] || (tUi["Sistem akan menggambar ulang setiap teks dan komponen PDF menjadi gambar statis beresolusi tinggi, membuat dokumen ini 100% aman dari proses pengeditan atau salin-tempel."] || "Sistem akan menggambar ulang setiap teks dan komponen PDF menjadi gambar statis beresolusi tinggi, membuat dokumen ini 100% aman dari proses pengeditan atau salin-tempel.")}</p>
        </div>
      </div>
      ...
      >
        {isProcessing ? (tUi["Memproses Rasterisasi..."] || "Memproses Rasterisasi...") : (tUi["Jadikan PDF Permanen Sekarang"] || (tUi["Jadikan PDF Permanen Sekarang"] || "Jadikan PDF Permanen Sekarang"))}
      </button>
    </div>
  );
};
