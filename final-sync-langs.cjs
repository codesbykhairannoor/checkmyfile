...
async function main() {
  ...
  // 1. Inject into baseDict if missing
  let baseDictIndex = lines.findIndex(l => l.includes('const baseDict: UiDictionary = {'));
  if (baseDictIndex !== -1) {
    let hasKeys = false;
    for (let i = baseDictIndex; i < lines.length; i++) {
    ...
    if (!hasKeys) {
      console.log('Injecting keys into baseDict (English)...');
      const insertion = Object.entries(PAGES_DICTIONARY).map(([k, v]) => `  ${k}: '${v.replace(/'/g, "\\'")}',`);
      lines.splice(baseDictIndex + 1, 0, ...insertion);
    }
  }

  // 2. Check all 29 languages
  for (let lang of langs) {
    const blockStartRegex = new RegExp(`^  ${lang}:\\s*\\{`);
    let blockStartIndex = lines.findIndex(l => blockStartRegex.test(l));
    
    if (blockStartIndex === -1) {
      console.log(`Language block ${lang} not found! Something is very wrong.`);
      continue;
    }
